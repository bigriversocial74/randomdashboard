...
{
    $steps=array_values(array_filter(process_mapping_demo_steps(),fn($s)=>(int)$s['version_id']===1));$current=$steps[4]??$steps[0];return[['id'=>1,'process_id'=>1,'version_id'=>1,'instance_number'=>'P2P-DEMO-001','entity_id'=>5,'canonical_record_type'=>'purchase_order','canonical_record_id'=>2,'instance_title'=>'Critical power replenishment purchase','status'=>'in_progress','current_step_id'=>$current['id'],'started_by'=>5,'started_at'=>date('Y-m-d H:i:s',strtotime('-2 days')),'due_at'=>date('Y-m-d H:i:s',strtotime('+2 days')),'completed_at'=>null,'cycle_seconds'=>0,'process_data_json'=>json_encode(['supplier_id'=>1,'purchase_order_id'=>2]),'created_at'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s')]];
}
function process_mapping_demo_step_instances(): array
{
    $rows=[];$steps=array_values(array_filter(process_mapping_demo_steps(),fn($s)=>(int)$s['version_id']===1));foreach($steps as$i=>$s){$status=$i<4?'completed':($i===4?'active':'pending');$rows[]=['id'=>$i+1,'instance_id'=>1,'step_id'=>$s['id'],'assigned_entity_id'=>null,'assigned_user_id'=>$i<4?5:3,'status'=>$status,'started_at'=>$i<=4?date('Y-m-d H:i:s',strtotime('-'.(48-$i*8).' hours')):null,'due_at'=>$s['sla_minutes']>0?date('Y-m-d H:i:s',strtotime('+'.max(1,$s['sla_minutes']).' minutes')):null,'completed_at'=>$i<4?date('Y-m-d H:i:s',strtotime('-'.(40-$i*8).' hours')):null,'elapsed_seconds'=>$i<4?28800:0,'evidence_note'=>$i<4?'Seeded completion evidence.':'','output_json'=>'{}','created_at'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s')];}return$rows;
}
