...
function process_mapping_template_catalog(): array
{
    return [
        'procure_to_pay' => [
            'name'=>'Procure-to-Pay','category'=>'source_to_pay',
            ...
            'lanes'=>[
                ['code'=>'requesting_business','name'=>'Requesting Business','participant_type'=>'entity','entity_code'=>'@initiating','role_code'=>'requester'],
                ...
                ['code'=>'receiving_business','name'=>'Receiving Business','participant_type'=>'entity','entity_code'=>'@initiating','role_code'=>'receiver'],
...
{
    $lanes = [];
    foreach ($laneNames as $index => $laneName) {
        $lower = strtolower($laneName);
        $entityCode = str_contains($lower, 'shared procurement') ? 'GRUBER-PROCUREMENT'
            : (str_contains($lower, 'shared finance') || str_contains($lower, 'accounts payable') ? 'GRUBER-ACCOUNTS-PAYABLE' : null);
        $participantType = str_contains($lower, 'supplier') ? 'external' : ($entityCode ? 'entity' : 'role');
        $lanes[] = [
            'code'=>'lane_'.($index + 1),'name'=>$laneName,'participant_type'=>$participantType,
            'entity_code'=>$entityCode,'role_code'=>strtolower(str_replace([' ','/','&'], ['_','','and'], $laneName)),
        ];
    }
    $steps = [];
    $transitions = [];
    ...
    foreach ($stepNames as $index => $stepName) {
        $lane = 'lane_'.(($index % $laneCount) + 1);
        $type = $index === 0 ? 'start'
            : ($index === count($stepNames) - 1 ? 'end'
                : (str_contains(strtolower($stepName), 'approve') || str_contains(strtolower($stepName), 'certif') ? 'approval' : 'task'));
        $steps[] = [
            'code'=>'step_'.($index + 1),'name'=>$stepName,'type'=>$type,'lane'=>$lane,
            'x'=>$x + ($index * 185),'y'=>70 + (($index % $laneCount) * 160),'permission'=>'platform.view',
            'sla'=>$type === 'start' || $type === 'end' ? 0 : 480,'evidence'=>$type !== 'start' && $type !== 'end',
            'record'=>'business_process',
        ];
        if ($index > 0) $transitions[] = ['from'=>'step_'.$index,'to'=>'step_'.($index + 1),'label'=>'Continue','default'=>true];
    }
    return [
        'name'=>$name,'category'=>$category,'value_chain'=>$valueChain,
        'purpose'=>'Reusable governed '.$name.' operating model.','modules'=>$modules,
        'lanes'=>$lanes,'steps'=>$steps,'transitions'=>$transitions,
    ];
}
...
function process_mapping_template_preview(string $code): array
{
    ...
    $template = $catalog[$code] ?? null;
    if (!$template) return ['valid'=>false,'errors'=>['Unknown process template.']];
    $stepCodes = array_column($template['steps'], 'code');
    $errors = [];
    if (count($stepCodes) !== count(array_unique($stepCodes))) $errors[] = 'Step codes must be unique.';
    foreach ($template['transitions'] as $transition) {
        if (!in_array($transition['from'], $stepCodes, true) || !in_array($transition['to'], $stepCodes, true)) {
            $errors[] = 'Every transition must reference valid steps.';
        }
    }
    return [
        'valid'=>$errors === [],'errors'=>$errors,'code'=>$code,'name'=>$template['name'],
        'purpose'=>$template['purpose'],'value_chain'=>$template['value_chain'],'modules'=>$template['modules'],
        'lane_count'=>count($template['lanes']),'step_count'=>count($template['steps']),
        'transition_count'=>count($template['transitions']),'template'=>$template,
    ];
}
