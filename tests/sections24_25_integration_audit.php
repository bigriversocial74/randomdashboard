...
$companyOnePo=null;$companyTwoPo=null;foreach(data_collection('purchase_orders')as$po){if((int)$po['company_id']===1&&(int)$po['id']!==2&&$companyOnePo===null)$companyOnePo=$po;if((int)$po['company_id']===2&&$companyTwoPo===null)$companyTwoPo=$po;}
s2425($companyOnePo!==null&&$companyTwoPo!==null,'Audit requires independent Company 1 and Company 2 purchase orders.');
$companyOneInstance=process_mapping_start_instance(1,(int)$codes['COMP-GC'],'purchase_order',(int)$companyOnePo['id'],'Company 1 procure-to-pay audit','Verified canonical Company 1 purchase order and process assignment.');
$steps=process_mapping_steps((int)$companyOneInstance['version_id']);$lanes=process_mapping_lanes((int)$companyOneInstance['version_id']);$laneMap=[];foreach($lanes as$l)$laneMap[(int)$l['id']]=$l;$stepInstances=process_mapping_step_instances((int)$companyOneInstance['id']);$assigned=[];foreach($stepInstances as$si){$step=process_mapping_find_step((int)$si['step_id']);$lane=$laneMap[(int)$step['lane_id']];$assigned[$lane['lane_code']]=$si['assigned_entity_id'];}
...
$companyTwoInstance=process_mapping_start_instance(1,(int)$codes['COMP-GPS'],'purchase_order',(int)$companyTwoPo['id'],'Company 2 permission audit','Verified canonical Company 2 purchase order and process assignment.');
...
s2425(process_mapping_find_instance(1)!==null,'Company 1/4/6 administrator must see the seeded Company 1 process instance.');
s2425(process_mapping_find_instance((int)$companyTwoInstance['id'])===null,'Company 1/4/6 administrator must not see the Company 2 process instance.');
...
s2425_user(5,2);
s2425(process_mapping_find_instance(1)===null,'Company 2/3/5 contributor must not see the seeded Company 1 process instance.');
s2425(process_mapping_find_instance((int)$companyTwoInstance['id'])!==null,'Company 2/3/5 contributor must see the Company 2 process instance.');
$active=array_values(array_filter(process_mapping_step_instances((int)$companyTwoInstance['id']),static fn(array$r):bool=>$r['status']==='active'));s2425(count($active)===1,'Company 2 instance must expose exactly one active start step.');
$companyTwoInstance=process_mapping_advance_step((int)$active[0]['id'],'complete','Start event confirmed by the requesting business.');
$active=array_values(array_filter(process_mapping_step_instances((int)$companyTwoInstance['id']),static fn(array$r):bool=>$r['status']==='active'));$companyTwoInstance=process_mapping_advance_step((int)$active[0]['id'],'complete','Purchase request created with supporting evidence.');
$active=array_values(array_filter(process_mapping_step_instances((int)$companyTwoInstance['id']),static fn(array$r):bool=>$r['status']==='active'));s2425_expect_runtime(fn()=>process_mapping_advance_step((int)$active[0]['id'],'complete','Attempt without approval review permission.'),'A user lacking the step permission must not advance the process.');

s2425_user(3,2);
$advanced=process_mapping_advance_step((int)$active[0]['id'],'complete','Budget validation independently reviewed.');
s2425($advanced['status']==='in_progress','Authorized procurement manager must advance the governed step.');
