...
<section class="notice-card executive-briefing-notice"><div><span class="notice-icon">▥</span><div><strong>Enterprise spend and category planning</strong><p>Move from operational reports into governed spend classifications, category strategies, procurement plans, and plan-versus-actual controls.</p></div></div><a href="<?= h(app_url('spend-strategy.php')) ?>">Open Spend Strategy →</a></section>
...
<section class="panel report-detail-panel" id="reportDetail">
    <header class="panel-head"><div><span class="eyebrow"><?= h($report['module']) ?> · <?= h($report['scope']) ?></span><h2><?= h($report['title']) ?></h2><p><?= h($report['description']) ?></p></div><span class="panel-meta"><?= count($report['rows']) ?> rows · Generated <?= h(date_us($report['generated_at'], true)) ?></span></header>
    <?php if (!$report['rows']): ?><?php render_empty_state('No report rows', 'No records are visible for this report in the current company and permission scope.'); ?>
    <?php else: ?><div class="table-wrap"><table class="data-table" data-table><caption class="sr-only"><?= h($report['title']) ?> for <?= h($report['scope']) ?></caption><thead><tr><?php foreach ($report['columns'] as $column): ?><th scope="col" data-sort><?= h($column) ?></th><?php endforeach; ?></tr></thead><tbody><?php foreach ($report['rows'] as $row): ?><tr><?php foreach ($row as $value): ?><td><?= h((string)$value) ?></td><?php endforeach; ?></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</section>
<?php render_app_end(); ?>
