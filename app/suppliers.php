...
$actions='<a class="button secondary" href="'.h(app_url('sourcing.php')).'">Compare Suppliers</a><a class="button secondary" href="'.h(app_url('contracts.php')).'">Contracts & Renewals</a>';
...
<tr><td><strong><?= h($supplier['name']) ?></strong><small><?= h($supplier['supplier_number']) ?> · <?= h($supplier['payment_terms']) ?> <?= sample_badge() ?></small></td><td><?= h($supplier['category']) ?></td><td><div class="company-chip-row"><?php foreach($supplier['company_ids'] as $cid): ?><span><?= h(data_find('companies',$cid)['code']??$cid) ?></span><?php endforeach; ?></div></td><td><?= h(data_user_name($supplier['owner_id'])) ?></td><td><?= badge($supplier['risk']) ?><small><?= badge($supplier['status']) ?></small></td><td><?= badge($supplier['review_status']) ?></td><td class="numeric"><strong><?= money($supplier['annual_spend']) ?></strong></td><td><div class="contract-actions"><a class="mini-button" href="<?= h(app_url('contracts.php?supplier_id='.(int)$supplier['id'])) ?>">Contracts</a><?= workflow_actions('suppliers',(int)$supplier['id'],$supplier['review_status'],'suppliers','suppliers.php') ?></div></td></tr>
<?php endforeach; ?>
...
<section class="panel"><header class="panel-head"><div><span class="eyebrow">Commercial terms</span><h2>Contracts and renewals</h2></div><span class="panel-meta"><?= count($contracts) ?> agreements</span></header><div class="compact-list"><?php foreach($contracts as $contract): ?><article><div><strong><a href="<?= h(app_url('contracts.php?id='.(int)$contract['id'])) ?>"><?= h($contract['title']) ?></a></strong><small><?= h(data_supplier_name($contract['supplier_id'])) ?> · <?= h($contract['number']) ?> · Ends <?= h(date_us($contract['end_date'])) ?></small></div><div class="right-summary"><?= badge($contract['status']) ?><b><?= compact_money($contract['annual_value']) ?></b></div></article><?php endforeach; ?><?php if(!$contracts): render_empty_state('No contracts','No supplier agreements are visible in this scope.'); endif; ?></div></section>
